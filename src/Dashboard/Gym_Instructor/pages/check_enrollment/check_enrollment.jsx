...
function CheckEnrollment() {
  ...
  const fetchInfo = async () => {
    return await fetch(SERVER_ROOT_PATH + "/check_gym_enrollment")
  ...
  const fetchPendingInfo = async () => {
    return await fetch(SERVER_ROOT_PATH + "/pending_gym_enrollment")
      .then((response) => response.json())
  ...
  const onClickAccept = async () => {
    ...
    try {
      const response = await fetch(
        SERVER_ROOT_PATH + "/accept_gym_enrollment",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="gym-check-enrollment-container">
      <div className="enrollment-container">
        <h3>number of current enrollments : {message.length - 1}</h3>
        <Table noOfRows={message.length} noOfColumns={3} rowEntries={message} />
        <h3>pending requests</h3>
        <Table
          noOfRows={pendingMessage.length}
          noOfColumns={3}
          rowEntries={pendingMessage}
        />
        <div className="enrollment_accept">
          <h4>enter the row no. of the enrollment you wish to accept:</h4>
          <select
            onChange={(e) => {
              setUsername(pendingMessage[e.target.value][0]);
              setYear(pendingMessage[e.target.value][1]);
              setMonth(pendingMessage[e.target.value][2]);
              console.log(username);
            }}
            style={{ width: "200px" }}
          >
          ...
          <button
            ...
            value="book"
            textColor={1}
            className="gym_instructor_page_user_button"
